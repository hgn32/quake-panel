...
- 画像の**色から震度値を復元しない** (§2(2)、検討書の Phase 4 は除外済み)。
  延岡付近の実測震度は、気象庁発表の値 (P2P 551) を履歴パネルに出している。
...
| `HOME_NAME` / `HOME_LAT` / `HOME_LON` | 宮崎県延岡市 | 利用地。地図の中心と履歴の強調に使う |
| `TSUNAMI_HOME_AREAS` | `宮崎県` | 津波予報で強調する予報区 (カンマ区切り) |
| `KMONI_IDLE_FRAME_INTERVAL_MS` | `1000` | 平常時の画像取得間隔。負荷を抑えたいなら `2000` |
| `KMONI_ACTIVE_FRAME_INTERVAL_MS` | `1000` | EEW 発表中の画像取得間隔 |
...
localStorage に端末ごとに保存される。サーバーの挙動は変わらない。
