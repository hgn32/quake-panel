...
設定画面は **「音と明滅」「地図」「利用地」「地震情報」の 4 章**に分けてある。
見出しと「保存 / 取消」は固定で、設定の並びだけがスクロールする。

- **EEW の通知範囲**: 「予報から通知」/「警報のみ通知」(§1 の端末別切替)。
  サーバーは常に予報も警報も配信し、鳴らすかどうかは端末側で決める。
- 音量 / テスト (実際の警報と同じ音と画面明滅を数秒出す)
- **地図に出す指標**: 既定はサーバーの `KMONI_LAYER` に従う。この端末だけ変えることもでき、
  そのときサーバーは**その指標も追加で取りに行く** (見ている端末が無くなれば止める)。
  いま何を出しているかは地図右下の凡例に常に出る
- **音を鳴らす時間 / 画面明滅を続ける時間**: 既定は音 10 秒・明滅 60 秒。
  発表が続いていても、この時間で音と明滅だけ止める (遠方の地震で長時間
...
- **地図と地震情報の境目**: 設定画面ではなく**境目そのものをドラッグ**して動かす。
  横並びなら左右、縦並び (スマホ・狭い画面) なら上下。動かした位置はその端末に保存され、
  次に開いても同じ配分になる。ダブルクリックで既定に戻り、矢印キーでも動かせる。
  読めなくなる幅・地図が潰れる幅にはならないよう、画面の大きさに応じて範囲を制限している
  (`shared/src/layout.ts`)
- **表示位置を固定**: 常時表示の端末で誤って動かさないためのキオスク向け設定。
  地図の操作に加えて、上の境目のドラッグも受け付けなくなる
- 観測点の発光表示 (重い端末では切る)
- 履歴の表示件数
...
- 都道府県は、地震情報では**観測点の県**、緊急地震速報では**予想震度が出ている地域**で見る。
  予想震度の地域がまだ来ていない第一報は、地域では落とさない (震度の条件だけ見る)
- **取消報と「発表なし」は絞り込みに関わらず必ず流す。** ここで黙ると、
  自動化で点けた照明やダッシュボードが戻せなくなる。
  同じ理由で、一度流した地震の続報は下方修正で条件を外れても流し続ける
