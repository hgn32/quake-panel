...
### 予想震度・予測円画像の中身、EEW JSON の続く長さ (2026-09-02 実測、2026-09-07 訂正)

対象: 2026/09/02 08:24 日向灘 M3.6 (report_id `20260902082446`)。GIF をデコードして
画素単位で確認した。

- **予想震度画像 (EstShindoImg) は画素ごとに値を持つ「面」である**。352x400 の中に
  観測点ごとの点が並んでいるのではなく、地理的な範囲を隙間なく塗った面として
  配信されている。2026-09-02 日向灘 M3.6 の時点では不透明画素が 1800 個だけで、
  同時刻のリアルタイム震度画像 (RealTimeImg) の観測点の点と **97.6% が 2px 以内**、
  **52% は完全一致**していたため「観測点ごとの点」と誤認していたが、これは面が
  観測点の分布する陸域に載っているためであって、点であることの根拠にはならない。
  規模の大きい EEW で再検証すると誤りが確定した (次項)。
- **再検証 (2026-09-07 23:21 熊本県天草・芦北地方の EEW、
  `EstShindoImg/eew/20260907/20260907232200.eew.gif`)**: 不透明画素 **2,517 個**が
  x=1..69 / y=296..383 の範囲に**連続して隙間なく**並び、隣り合う画素で値が
...
- **予測円画像 (PSWaveImg) には P 波 (赤) / S 波 (青) の円のほかに、震央に
  ...
  `shared/src/kmoniGeo.ts`) を描画から除いている。
- **見出し文字が焼き込まれているのはリアルタイム震度画像だけ**。予想震度・予測円の
  不透明画素は色データのみで、黒文字 (リアルタイム震度画像で 1230 画素) を含まない。
  したがって予想震度・予測円では見出し帯 (`KMONI_CAPTION_BOX`) を避ける必要が無い。
- **較正の裏取り**: 予測円 (P 波) の中心を最小二乗で求めると (64.94, 352.56)。
  `projectToPixel(31.9, 131.8)` (震央) は (64.61, 351.31) で、差は 0.3px / 1.3px。
  §1-4 の較正が予測円の実際の中心とも一致することを確認した。
- **EEW JSON (§1-2) は最終報のあとも同じ内容を約3.5分返し続ける**。
  最終報 08:25:26 のあと、08:28:00〜08:28:30 の間で「データがありません」に変化した
  (予想震度・予測円の画像も同じ頃 302 に変わる)。したがって「最終報を受けた」ことは
  「もうすぐ配信が止まる」ことを意味するが、「今すぐ止まる」わけではない。
  この性質があるため、EEW の保持期限は「画面が受け取った時刻」ではなく
  「内容が動いた時刻」から数える必要がある (`server/src/eew/coordinator.ts`)。

## 1-4. 座標系 (投影パラメータ)

kmoni は投影パラメータを公開していないため、較正した値を使う。
`shared/src/kmoniGeo.ts` の `KMONI_MAP` / `KMONI_INSET` が現在値。

### 現行較正 (2026-09-02、観測点リストとの対応による再推定)

防災科研の公式観測点リスト (K-NET + KiK-net、1,047 + 702 = **1,749 点**) を入手し、
配信画像から分離した観測点位置と突き合わせて再推定した。

- **入手元**: <https://www.kyoshin.bosai.go.jp/ja/stationlist/> (要ユーザー登録)。
  CSV をダウンロードし、`node scripts/build-station-list.mjs --source <CSV のパス>`
  で配信用 JSON (`client/public/assets/kmoni-stations.json`) に変換する。
  配信画像そのものは加工しない (別途公開されている観測点情報を使うだけ)。
- **裏取り**: リストの緯度経度は kmoni の描画位置と一致する。画像から分離できた
  1,366 点すべてがリストの点と**最大 2.86px 以内 (中央値 1.30px、3px 超はゼロ)**。
- **再較正**: その対応 1,063 点で投影パラメータを最小二乗で再推定。

```
投影: 正距円筒 (回転なし、円錐投影ではない)
  x = (経度 - 128.6010) * 20.3532
  y = (46.2600 - 緯度) * 24.5432
南西諸島インセット: 同縮尺の平行移動 (dx=+123.75, dy=-349.0)
```

- **残差**: 本土側 RMS 経度方向 0.353px / 緯度方向 0.340px
  (旧: 島嶼 15 点による較正で 0.72px / 0.49px)。
  インセット側は対応 15 点で RMS 0.565px / 0.589px。
- **独立検証**: 2026-09-02 08:24 日向灘 EEW の P 波予測円の中心 (実測 64.94, 352.56)
  と `projectToPixel(震央)` の差が、旧較正 1.3px → 新較正 **0.2px** に改善 (§1-3)。
- **中心画素の丸め規則**: 観測点の中心画素は `Math.floor(x)`, `Math.floor(y)` で
  **78.8% が完全一致**。残りは kmoni 側の描画丸めで ±1px ずれる
  (`client/src/core/mapView.ts` の `samplePoints` は周囲 8 画素まで見て救っている)。
  見出し帯 (`KMONI_CAPTION_BOX`) に入る観測点・画像の外に出る観測点は共に **0 点**。

### 旧較正 (2026-08-13 時点、参考)

基図 (陰影) とオープンな行政区域データを重ねて較正していた。

```
x = (経度 - 128.6169) * 20.2976
y = (46.2239 - 緯度) * 24.5262
南西諸島インセット: dx=+124.585, dy=-345.334
```

島嶼 15 点の対応による残差 RMS は経度方向 0.72px / 緯度方向 0.49px。
手順は `scripts/calibrate-kmoni-map.py` に残してある (観測点リストが使えない
環境でのフォールバック用)。較正でつまずいた点を 1 つ記録しておく。基図の陰影は
**山地にしか付いておらず、平野部は海と同じ白**である。陸地ポリゴンと灰色部分を
単純に比較すると平野の分だけ縮尺が小さい方に引っ張られ、誤った値
(px/度 が 25% 小さい) に収束する。リアルタイム震度画像の観測点は平野にも分布
しているので、これを併用すると解消する。
