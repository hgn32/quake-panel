...
### 予想震度・予測円画像の中身、EEW JSON の続く長さ (2026-09-02 実測)

対象: 2026/09/02 08:24 日向灘 M3.6 (report_id `20260902082446`)。GIF をデコードして
画素単位で確認した。

- **予想震度画像 (EstShindoImg) は面ではなく点**。352x400 の中に観測点ごとの
  2x2px の点として描かれている。同時刻のリアルタイム震度画像 (RealTimeImg) と
  比較すると、不透明画素 1800 個の **97.6% が観測点の点と 2px 以内**、
  **52% は完全一致**。表示側で画像ごと拡大すると、この点が倍率ぶんの塊ににじみ、
  海岸線からはみ出して見える (不具合の原因)。
