...
describe('HA へ通知する地震情報の絞り込み', () => {
  ...
  it('最大震度のしきい値で落とす', () => {
    const filter = { minIntensity: 30, prefectures: [] };
  ...
  it('しきい値があるとき、震度不明の電文は落とす', () => {
    const filter = { minIntensity: 10, prefectures: [] };
  ...
  it('県名の表記ゆれ (宮崎 / 宮崎県) を拾う', () => {
    const filter = { minIntensity: 0, prefectures: ['宮崎'] };
    assert.equal(shouldNotifyQuake(quake(30, [point('宮崎県', 30)]), filter), true);
  });

  it('震度と地域の両方を満たすときだけ流す', () => {
    const filter = { minIntensity: 40, prefectures: ['宮崎県'] };
...
describe('HA へ通知する緊急地震速報の絞り込み', () => {
  ...
  it('予想最大震度のしきい値で落とす', () => {
    const filter = { minIntensity: 45, prefectures: [] };
